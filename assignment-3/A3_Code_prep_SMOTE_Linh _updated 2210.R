...
bank_raw <- read.csv("C:/Users/linhk/OneDrive/Documents/Swinbrune/Smester 04/AI/ASS3/archive (7)/bank-additional-full.csv",
...
if (duplicates > 0) {
  ...
  write.csv(dup_rows, "C:/Users/linhk/OneDrive/Documents/Swinbrune/Smester 04/AI/ASS3/archive (7)/bank-additional-full.csv", row.names = FALSE)
...
if ("duration" %in% names(bank_pp)) 
 
bank_pp <- bank_pp[ , !(names(bank_pp) == "duration")]
# 2.3 Imputation
...
write.csv(
  data_clean,
  "C:/Users/linhk/OneDrive/Documents/Swinbrune/Smester 04/AI/ASS3/archive (6)/bank-additional-full.csv",
  row.names = FALSE)
...
# (Optional) write to CSV
write.csv(bank_model_df_z, "C:/Users/linhk/OneDrive/Documents/Swinbrune/Smester 04/AI/ASS3/archive (6)/bank-additional-full.csv", row.names = FALSE)
write.csv(bank_model_df_mm, "C:/Users/linhk/OneDrive/Documents/Swinbrune/Smester 04/AI/ASS3/archive (6)/bank-additional-full.csv", row.names = FALSE)
...
apply_smote <- function(data, name) { #make a function called "apply_smote" to reuse
  cat("\nProcessing:", name, "\n") #printing the dataset name so we know what we are working on. 
  
  # Show BEFORE SMOTE
  class_counts_before <- table(data$y_binary) #count how many rows are in each class in your target column (y_binary)
  cat("  BEFORE SMOTE:\n")
  cat("    Class 0:", class_counts_before[1], "\n") #print the number of samples for each class and shows the imbalance ratio. 
  cat("    Class 1:", class_counts_before[2], "\n")
  cat("    Imbalance ratio:", round(class_counts_before[1] / class_counts_before[2], 2), ": 1\n")
  
  # Separate features and target
  X <- data[, !names(data) %in% "y_binary", drop = FALSE] #splitting data into features (x) and output (target, y)
  y <- as.numeric(data$y_binary)  # ensure numeric 0/1 #make sure target is numeric (0 and 1)
  
  # Apply SMOTE (K=5 nearest neighbors) on FULL data, reducing class imbalance, which help ML models perform better. 
  smote_result <- smotefamily::SMOTE(X = X, target = y, K = 5, dup_size = 0) # Use SMOTE to create extra synthetic samples for the minority class 
  
  # Combine back together
  data_smote <- smote_result$data #put the new data together and renames the last column to y_binary to match your original target name. 
  colnames(data_smote)[ncol(data_smote)] <- "y_binary"
  
  # Ensure 0/1 numeric for target (SMOTE may return numeric not exactly 0/1), because SMOTE works by creating new samples for the minority class, using 0 and 1 make it clear and simple for SMOTE to balance your classes. 
...
num_folds <- 5
#---Run 5-fold CV without SMOTE
# Create indices for cross-validation
folds <- createFolds(bank_model_df_z$y_binary, k = num_folds)

#Make sure target variable is a factor before creating folds, telling R to treat the outcomes as clear strategies: "no" (0) and "yes" (1), so it can correctly calculate probabilities and results for the positive class. If it stayed as numbers, R might not balance the splits—some groups could end up with only "yes" or only "no"
bank_model_df_z$y_binary <- factor(bank_model_df_z$y_binary, levels=c(0,1))

# Perform cross-validation
for (i in 1:num_folds) { #"for" loop runs once for each fold. 
  #split data
  train_data <-bank_model_df_z[-folds[[i]],]#the code picks most of the data the training set on each run, and [-folds[[i]],- leaving out the current fold to use as testing, and keeps all the rest for training the model .test_data <- bank_model_df_z_smote[folds[[i]],] #the code picks the left- out part as the test set, and [folds[[i]],]- selecting the part of data you want to test the model with in each round. 
  test_data <- bank_model_df_z[folds[[i]], ] 
  
  # Train model inside the loop
  log_reg_model <- glm(y_binary ~ ., data = train_data, family = binomial)
  
  # Prediction probabilities, giving the answer whether each person is predicted as 1 (positive) or 0 (negative)
  predicted_probs <- predict(log_reg_model, newdata = test_data, type = "response") #get probability for each test sample.
  predictions <- ifelse(predicted_probs > 0.5, 1, 0)# Convert to class predictions by turning probabilities above 0.5 in 1 (yes), otherwise o(No)
  
  #Make sure both are factors as it needs for the metrics like accuracy, precision and recall and confusionmatrix only work if both sides are factors. it avoids confusion and make the calculation of scores work properly. 
  predictions<-factor(predictions, levels=c(0,1))
  actuals<-factor(test_data$y_binary, levels=c(0,1))
  
  #Evaluation- comparing predictions to real answers by calculating these below metrics. 
  cm <- confusionMatrix(predictions, actuals, positive="1")
  acc      <- cm$overall["Accuracy"] #the proportion of correct predictions (both positive and negative)- answered" How often was the model right overall"
  precision<- cm$byClass["Precision"] # measures how many cases predicted as positive are actually positive- answered "if the model says "yes", how often is it correct?"
  recall   <- cm$byClass["Recall"] #measures how many actual positive cases the model successfully found- answered "out of very real "yes", how many did the model find?"
  f1       <- cm$byClass["F1"] # combine precision and recall into one value (harmonic value)- answered "how balanced is the model between catching positives and being accurate when it says "yes". 
...
#---Run 5-fold CV with SMOTE
# Create indices for cross-validation
folds <- createFolds(bank_model_df_z_smote$y_binary, k = num_folds)

#Make sure target variable is a factor before creating folds, telling R to treat the outcomes as clear strategies: "no" (0) and "yes" (1), so it can correctly calculate probabilities and results for the positive class. If it stayed as numbers, R might not balance the splits—some groups could end up with only "yes" or only "no"
bank_model_df_z_smote$y_binary <- factor(bank_model_df_z_smote$y_binary, levels=c(0,1))

# Perform cross-validation
for (i in 1:num_folds) { #"for" loop runs once for each fold. 
#split data
train_data <-bank_model_df_z_smote[-folds[[i]],]#the code picks most of the data the training set on each run, and [-folds[[i]],- leaving out the current fold to use as testing, and keeps all the rest for training the model .test_data <- bank_model_df_z_smote[folds[[i]],] #the code picks the left- out part as the test set, and [folds[[i]],]- selecting the part of data you want to test the model with in each round. 
test_data <- bank_model_df_z_smote[folds[[i]], ] 
...
#3. Logistic Regression _ Threshold Optimization

# Train your model (assume already fitted)
# log_reg_model <- glm(y_binary ~ ., data=train_data, family=binomial)


#Make sure the target is a factor (for fair folds)
bank_model_df_z_smote$y_binary <- factor(bank_model_df_z_smote$y_binary, levels=c(0,1))
#Define number of folds
num_folds <- 5
...
#Prepare a range of threshold (0.3-0.7) and step=0.05
thresholds <- seq(0.3, 0.7, by=0.05)

#Set up a table store metric results for each threshold
results <- matrix(NA, nrow=length(thresholds), ncol=4) #set up the table to store metric results
colnames(results) <- c("Accuracy", "Precision", "Recall", "F1") #names the columns in the table. 

#For each threshold, getting ready to save accuracy, precision, recall and F1 scores for all folds.
for (ti in seq_along(thresholds)) {
  t <- thresholds[ti]
  accs <- pre <- rec <- f1m <- numeric(num_folds) 

#------Run threshold optimization 5- fold CV with SMOTE---
  
#Perform cross validation  
for (i in 1:num_folds) {
    
#Split data
train_data <- bank_model_df_z_smote[-folds[[i]], ]
test_data  <- bank_model_df_z_smote[ folds[[i]], ]
...
predicted_probs <- predict(log_reg_model, newdata=test_data, type="response") #Get predicted probabilities for the test data
predictions <- ifelse(predicted_probs > t, 1, 0) #covert probabilities into hard 1 or 0 labels using the current threshold being tested
predictions <- factor(predictions, levels=c(0,1)) # make sure predictions and actual labels are both treated as factors. 
actuals     <- factor(test_data$y_binary, levels=c(0,1))

#Evaluation    
cm <- confusionMatrix(predictions, actuals, positive="1")
accs[i] <- cm$overall["Accuracy"]
pre[i] <- cm$byClass["Precision"]
rec[i] <- cm$byClass["Recall"]
f1m[i] <- cm$byClass["F1"]
}
  
# Print results of each fold
results[ti, ] <- c(mean(accs), mean(pre), mean(rec), mean(f1m))
cat("Threshold:", t, "Avg Accuracy:", mean(accs),
      "Avg Precision:", mean(pre),
      "Avg Recall:", mean(rec),
      "Avg F1:", mean(f1m), "\n")
}

#---FEATURE SELECTION---
#Ensure target variable as numeric and included in numeric dataset.
bank_model_df_z_smote$y_binary <- as.numeric(bank_model_df_z_smote$y_binary)
numeric_cols <- bank_model_df_z_smote[sapply(bank_model_df_z_smote, is.numeric)]
colnames(numeric_cols) # y_binary should appear in the list

# Only use numeric columns (including y_binary for correlation)
numeric_cols <- bank_model_df_z_smote[sapply(bank_model_df_z_smote, is.numeric)]

# Calculate correlation matrix
correlations <- cor(numeric_cols, use = "complete.obs")

# Find correlations with y_binary (target column)
cor_with_target <- correlations["y_binary", ]
# Remove self-correlation
cor_with_target <- cor_with_target[names(cor_with_target) != "y_binary"]

# Sort correlations by absolute value (strongest factors first)
sorted_cor <- sort(abs(cor_with_target), decreasing = TRUE)

# Top 10 most correlated features with y_binary
top_features<- names(sorted_cor)[1:10]
top_values <- sorted_cor[1:10]

# Print top 10 features and their correlation
cat("Top 10 features correlated with y_binary:\n")
for (i in 1:10) {
  cat(i, ". ", top_features[i], ":", round(top_values[i], 3), "\n", sep = "")}

top_10_factors <- data.frame(
  variable = names(sorted_cor)[1:10],
  correlation = sorted_cor[1:10]
)

# Then plot using ggplot
p1 <- ggplot(top_10_factors, aes(x = reorder(variable, abs(correlation)),
                                 y = correlation,
                                 fill = correlation > 0)) +
  ...
  labs(title = "Top 10 Factors Correlated with y_binary",
       x = "Variables",
       y = "Correlation Coefficient",
       subtitle = "Ordered by absolute correlation strength") +
  geom_text(aes(label = round(correlation, 3)),
            hjust = ifelse(top_10_factors$correlation > 0, -0.1, 1.1), size = 3.5) +
  theme_minimal()

print(p1)

# List of top 10 feature names (from your plot)
top10_names <- c("nr.employed", "euribor3m", "emp.var.rate", "economic_situationGrowing",
                 "pdays", "poutcomesuccess", "days_since_contactRecent_Week", "previous",
                 "contacttelephone", "poutcomenonexistent")

# Create new data set with target and top 10 predictors
bank_top10_df <- bank_model_df_z_smote[, c("y_binary", top10_names)]

# Check result: shows only desired columns
str(bank_top10_df)


#2. Cross_ validation-> repeating this process over several different splits to give a more reliable, stable, and fair measure of the model's performance on unseen data

# Define number of folds
num_folds <- 5
#Make sure target variable is a factor before creating folds, telling R to treat the outcomes as clear strategies: "no" (0) and "yes" (1), so it can correctly calculate probabilities and results for the positive class. If it stayed as numbers, R might not balance the splits—some groups could end up with only "yes" or only "no"

bank_top10_df$y_binary <- factor(bank_top10_df$y_binary, levels=c(0,1))

# Create indices for cross-validation
folds <- createFolds(bank_top10_df$y_binary, k=5, list=TRUE, returnTrain=TRUE)

# Perform cross-validation
for (i in 1:num_folds) { #"for" loop runs once for each fold. 
  
  #split data
  train_data <-bank_top10_df[-folds[[i]],]#the code picks most of the data the training set on each run, and [-folds[[i]],- leaving out the current fold to use as testing, and keeps all the rest for training the model .test_data <- bank_model_df_z_smote[folds[[i]],] #the code picks the left- out part as the test set, and [folds[[i]],]- selecting the part of data you want to test the model with in each round. 
  test_data <- bank_top10_df[folds[[i]], ]    
  # Train model inside the loop
  log_reg_model <- glm(y_binary ~ ., data = train_data, family = binomial)
