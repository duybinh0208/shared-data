...
#   "nnet", "pROC", "RColorBrewer", "dplyr", "randomForest", "caTools", "neuralnet"
# ))
...
plot_roc_with_thresholds <- function(
  ...
  file_name <- sprintf("ROC_curve_%s.png", name)
  png(file_name, width = 800, height = 800, res = 100)
...
do_cross_validation_and_calcualate_auc <- function(case_label, source_df, data_name, smote_mode, model_type) {
  ...
  # One-hot encode can cause the column names to contain hyphens, this is used to change from hyphens to dots
  ...
  for (i in 1:num_folds) {
    fold_name <- sprintf("%s_fold_%d", case_label, i)
    ...
    plot_roc_with_thresholds(roc_obj, thresholds = c(0.2, 0.4, 0.5, 0.6, 0.8), name = sprintf("%s_%s", model_type, fold_name))
  }
...
# Run with all data
cat("\n[Case 01] - Run Logistic Regression model with SMOTE for the whole dataset, before doing the cross validations")
do_cross_validation_and_calcualate_auc(case_label = "case_01", source_df = bank_model_df_z, data_name = "bank_model_df_z", smote_mode = "before_cross_validaton", model_type = "Logistic_Regression")

cat("\n[Case 02] - Run Logistic Regression model with SMOTE for training data to prevent data leakage")
do_cross_validation_and_calcualate_auc(case_label = "case_02", source_df = bank_model_df_z, data_name = "bank_model_df_z", smote_mode = "in_cross_validaton", model_type = "Logistic_Regression")

cat("\n[Case 03] - Run Logistic Regression model without SMOTE to see how model perform with the original data")
do_cross_validation_and_calcualate_auc(case_label = "case_03", source_df = bank_model_df_z, data_name = "bank_model_df_z", smote_mode = "no_smote", model_type = "Logistic_Regression")

# Run with top features data
cat("\n[Case 04] - Run Logistic Regression model for top features with SMOTE for the whole dataset, before doing the cross validations")
do_cross_validation_and_calcualate_auc(case_label = "case_04", source_df = bank_model_df_z_top_feature, data_name = "bank_model_df_z_top_feature", smote_mode = "before_cross_validaton", model_type = "Logistic_Regression")

cat("\n[Case 05] - Run Logistic Regression model for top features with SMOTE for training data to prevent data leakage")
do_cross_validation_and_calcualate_auc(case_label = "case_05", source_df = bank_model_df_z_top_feature, data_name = "bank_model_df_z_top_feature", smote_mode = "in_cross_validaton", model_type = "Logistic_Regression")

cat("\n[Case 06] - Run Logistic Regression model for top features without SMOTE to see how model perform with the original data")
do_cross_validation_and_calcualate_auc(case_label = "case_06", source_df = bank_model_df_z_top_feature, data_name = "bank_model_df_z_top_feature", smote_mode = "no_smote", model_type = "Logistic_Regression")
